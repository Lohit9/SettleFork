...
//     references should be qualified as `"Table.Field"` so the helper
//     can rewrite them to `(<alias>.row_data->>'Field')`.
//
//     PR ζ — bare/single-quoted refs whose name appears in EXACTLY ONE
//     table in the map are silently auto-qualified against that table.
//     This restores apply-time robustness for cross-table TFMs whose AI
//     output was generated under the legacy "use bare field name"
//     prompt. Refs whose bare name collides across 2+ tables still
//     throw `Cross-table transforms must use table-qualified field
//     references.`, with the colliding table names appended to the
