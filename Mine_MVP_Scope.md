...
| **schema_documents** | Metadata for uploaded supplementary files (DDL scripts, ERD diagrams, PDFs, documentation). The actual files live in Supabase Storage; this table tracks the reference. | Schema documents are stored, parsed, and merged into the structured schema. See `docs/investigations/inv2-pdf-schema-enrichment.md` for the parsing chain. |
