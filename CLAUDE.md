...
- **User identity enrichment** for Supabase row-fetches uses [`enrichWithUserIdentity`](lib/auth/users.ts) — the canonical helper that joins `profiles.full_name` and `auth.users.email` (via the `getAuthEmailsByIds` SECURITY DEFINER RPC) onto a row list keyed by a configurable `user_id` field. Adopted by 6 callers across 4 files: `getProjectMembers`, `getOrgMembersAvailableForProject` ([project-members.ts](lib/actions/project-members.ts)); `getOrgMembers`, `adminGetOrgMembers` ([organizations.ts](lib/actions/organizations.ts)); `getFixHistory` ([quality-fixes.ts](lib/actions/quality-fixes.ts)); `getOutputsPageData` ([outputs.ts](lib/actions/outputs.ts)). Default missing-user fallback is `null`; `adminGetOrgMembers` passes `{missingFallback: 'Unknown'}` to preserve its admin-UI literal-string contract. Live-join at read time — never denormalise display name into the source table.
