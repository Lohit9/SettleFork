...
> **Cycle 1 update (2026-04-27).** The original 4a-3 design surfaced the FK precheck contract directly in the Mapping UI via `DOMINANT` / `JOINED` picker headers and a disambiguation dropdown. Cycle 1 of the architectural rebuild retired that surface — see ["Disambiguation contract — retired by Cycle 1"](#disambiguation-contract--retired-by-cycle-1-2026-04-27) below for the full retirement narrative. The sub-sections that follow describe **current** post-Cycle-1 behavior; the original contract is preserved in the retirement subsection for lineage.
...
- Selected chips render as a single flat strip regardless of how many source tables are involved — no `DOMINANT` / `JOINED` section headers, no anchor-source emphasis. Source-table grouping in the picker dropdown follows the canonical alphabetical server order; chips appear in selection order. When all selected fields share one table the layout is unchanged from the same-table case.

### FK inference contract

`createFieldMapping` always succeeds at create time for cross-table source sets (post-Cycle-1). The write path persists `mapping_sources.join_spec = null` unconditionally; FK metadata is re-derived on demand by `inferFkCandidates` (in `lib/utils/fk-inference.ts`) at read time for the drawer's "via `<fk_name>`" annotations and at apply time inside the Transform tab.

- **Mapping layer is precheck-free.** No FK validation runs on save; no `errorCode: 'CROSS_TABLE_AMBIGUOUS'` is returned; no `candidateFkFields` / `ambiguousJoinedTableId` / `dominantTableName` fields exist on the result shape; no disambiguation UI renders in the form.
- **Read-time inference** picks the single FK when only one candidate exists between two source tables. Multi-candidate ambiguity is silently tolerated at the Mapping layer — the drawer shows the source list without the "via …" annotation rather than blocking the save.
- **Apply-time surfacing.** When the user runs Apply Transform on a cross-table TFM whose joined sources have ambiguous or missing FKs, `applyTransform` raises `CROSS_TABLE_FK_INFERENCE_FAILED` from the Transform tab. That is the only surface where a user is asked to act on FK ambiguity post-Cycle-1.
...
### Disambiguation contract — retired by Cycle 1 (2026-04-27)

> **Historical narrative.** 4a-3 (2026-04-25) shipped cross-table mapping creation with an in-form FK precheck contract: the wrapper inspected candidate FKs per joined table and surfaced ambiguity directly in the Mapping drawer. Cycle 1 of the architectural rebuild (commit 2a514b6, deployed 2026-04-27) removed the entire surface — sources are now an unordered set at the Mapping layer, and any combination/join logic is the Transform tab's concern. The original disposition is preserved here for context; current behaviour is documented under **Picker behavior** and **FK inference contract** above.
>
> Original contract (now removed):
> - **Wrapper precheck** — `createFieldMapping` and `editMappingSources` ran `inferFkCandidates` per joined table and short-circuited with `errorCode: 'CROSS_TABLE_AMBIGUOUS'` when zero or multiple FKs were viable. Result shape carried `candidateFkFields`, `ambiguousJoinedTableId`, `ambiguousJoinedTableName`, and `dominantTableName`.
> - **Edit-mode dominant-swap guard** — `editMappingSources` raised `errorCode: 'DOMINANT_TABLE_CHANGED'` when the dominant (anchor) source table changed across an edit, blocking the save to avoid silent transform-SQL invalidation.
> - **Picker section headers** — `SourceFieldPicker` rendered selected chips under `DOMINANT` / `JOINED` small-caps section headers, with the first-picked source table fixed as dominant.
> - **JoinDisambiguation UI** — multi-candidate FK ambiguity surfaced as a native `<select>` dropdown under the picker; resolved rows rendered as read-only with a `Change` link. Form state carried `joinAnnotations` (the user's FK overrides) and `ambiguousCandidates` (the candidate lists per joined table).
>
> Why retired: founder direction during the architectural investigation — sources should be an unordered set at the Mapping layer with no anchor-source semantics, and FK inference belongs at apply time (Transform tab), not create time. The retirement nets ~500 LOC removed across the wrapper, form component, picker, drawer, and tests.
>
> What replaced it: nothing at the Mapping layer. FK inference still happens read-time (for "via `<fk_name>`" drawer annotations) and apply-time (Transform tab raises `CROSS_TABLE_FK_INFERENCE_FAILED` if a join cannot be inferred). The `joinAnnotations` input is preserved as a `@deprecated` no-op on `createFieldMapping` / `editMappingSources` for input-shape parity; it is never persisted.
>
> Forward pointer: Cycle 2 (deferred — gated on real customer migration with cross-table-spanning TFMs hitting a problem) would change the owning-TM rule from first-source-wins to fan-out and migrate the apply RPC accordingly. Cycle 1 stands as a defensible terminal state without it; production currently has 1 cross-table TFM (Epicor demo, never applied).
...
- `MappingContent` snapshots the dirty form state (`CreateMappingFormSnapshot { targetFieldId, selectedIds, combinationType }`) the moment the row changes, fires the toast, and threads the snapshot back via the drawer's `restoreFormState` prop on Undo. Restoration replays the URL drawer change, auto-activates the form on mount, and re-hydrates source selection in one render. Post-Cycle-1, `combinationType` is **derived** from `selectedIds.length` rather than stored as independent state (0 → `'custom_sql'`, 1 → `'single'`, 2+ → `'concat_space'`); edit mode preserves the original `editInitialState.combinationType` when ≥2 sources remain so existing concat variants are not silently rewritten. The retired `joinAnnotations` and `ambiguousCandidates` fields are no longer part of the snapshot shape — see ["Disambiguation contract — retired by Cycle 1"](#disambiguation-contract--retired-by-cycle-1-2026-04-27).
...
- **`editMappingSources(input)`** — atomic source-set replacement. Routes through the new `dq_replace_mapping_sources` RPC (idempotent DELETE + INSERT in one transaction) plus a `target_field_mappings` UPDATE for combination + status. Reverts status to `needs_review` on any source/combination change (§3.2). When sources change, also calls `resetFieldTransform` to delete the `transformations` row and revert staged data — combination-only edits do NOT reset the transform (§1.3). Post-Cycle-1 the wrapper does **not** run a cross-table FK precheck and does **not** raise on dominant-table swaps — sources are an unordered set at the Mapping layer (see ["Disambiguation contract — retired by Cycle 1"](#disambiguation-contract--retired-by-cycle-1-2026-04-27)). The error code union shrank from 9 to 7 members: `EditMappingErrorCode` is now (`VALIDATION`, `NOT_FOUND`, `PERMISSION_DENIED`, `MAINTENANCE_MODE`, `TFM_REJECTED`, `TFM_ACKNOWLEDGED`, `INTERNAL`) — `DOMINANT_TABLE_CHANGED` and `CROSS_TABLE_AMBIGUOUS` were removed.
- **`updateMappingCombination(tfmId, combinationType)`** — combination-only edit. Cheaper than `editMappingSources` because no RPC call is needed (sources unchanged). Status reverts to `needs_review`; transform is NOT reset (§1.3). Provenance laundering does not apply (§1f — no source touched). No-op short-circuit when the combination is unchanged AND status is already `needs_review` (avoids activity-log spam). Post-Cycle-1 the action is **unreachable from any UI surface** — the redesigned mapping page no longer surfaces combination-strategy editing as a standalone affordance (combination is derived from source-set cardinality at form level). The export persists for back-compat and continues to be exercised by `tests/actions/update-mapping-combination.test.ts` and the Heritage integration test, but no production call site invokes it.
...
#### AI-reasoning clearing on edit (PR A2)

`editMappingSources` clears the TFM-level `ai_reasoning` narrative to `null` on every edit — once a user has edited the mapping, the AI's prose describes a source→target pairing that no longer exists. The frozen first-AI-proposal copy survives in `target_field_mappings.original_ai_reasoning` (migration 083), so the clear is audit-safe.

This **supersedes the former 4a-4b "provenance laundering" rule** for the TFM-level column. That rule preserved `ai_reasoning` text whenever a surviving source row still carried AI provenance, to prevent audit-trail drift; migration 083's `original_ai_reasoning` now freezes the AI proposal independently, so clearing the live column outright is both audit-safe and consistent with every other edit path (`updateMappingSourceField`, `updateMappingTargetField`, `updateTransformSQL`, `autoSaveTransform`).

Per-source `mapping_sources.ai_reasoning` markers are **still laundered** server-side: a surviving AI-suggested source keeps its `'AI-suggested:…'` marker; new and originally-manual sources get the `'Manually selected by user'` marker. The form does NOT thread provenance flags over the wire (§7.1 — laundering is server-side only).
...
`mapping_combination_changed` is emitted by `updateMappingCombination` for combination-only edits (carries `previous_combination_type` + `combination_type`). Post-Cycle-1 this log entry is no longer produced in production traffic because no UI surface invokes `updateMappingCombination` (see the action description above); it remains plumbed for back-compat and integration-test coverage only.
...
- **`editInitialState?: EditMappingInitialState`** — `{ tfmId, selectedIds, combinationType, joinAnnotations }`. Required when `mode === 'edit'`. Provenance flags (`aiSuggested`, `originalSuggestedIds`) are intentionally absent — laundering is server-side (founder §1.f). Post-Cycle-1 the `joinAnnotations` field is preserved on the prop shape for input-shape parity with `createFieldMapping` / `editMappingSources` but is functionally a `@deprecated` no-op: it is read from `editInitialState`, never written by the form, and never persisted by the server (see ["Disambiguation contract — retired by Cycle 1"](#disambiguation-contract--retired-by-cycle-1-2026-04-27)).

Edit-mode behaviors:

- **State hydration** — `selectedIds` and `combinationType` initialize from `editInitialState` when in edit mode (§5.2). `joinAnnotations` is read from `editInitialState` for input-shape parity but ignored on the write path post-Cycle-1; the server's FK inference is read-time / apply-time only (see ["Disambiguation contract — retired by Cycle 1"](#disambiguation-contract--retired-by-cycle-1-2026-04-27)).
