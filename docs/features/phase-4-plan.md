...
**Status:** Phase 4a complete (2026-04-26, including 4a-6 cross-table apply). Phase 4b complete (2026-04-26 — 4b-1 W2 + W3 edit sources / combination shipped + 4b-2 W4 un-acknowledge shipped same day). Phase 4c complete (2026-04-26 — 4c-1 W5 bulk approve + high-confidence shipped same day; 4c-2 W5 bulk reject shipped 2026-04-26). Phase 4-polish-1 (visual restoration) shipped 2026-04-26; 4-polish-2 (group collapse) and 4-polish-3 (inline actions) pending. 4-extras pending. Created 2026-04-25; last updated 2026-04-26.
...
| 4c-2 | W5 remainder — bulk reject (kebab "Reject all needs-review", reuses `BulkConfirmDialog` with `mode='reject'`, per-TFM `resetFieldTransform` loop + bulk DELETE, partial-success forward-progress via `failedTfmIds`, single `mapping_bulk_rejected` activity log entry, `previewBulkReject` adds per-row `hasTransform` indicator) | ✅ shipped 2026-04-26 |
| 4-polish-1 | Visual restoration — `FieldMappingRow.tsx` 7-column grid (status dot / source table / source field / target / confidence / actions / chevron), `py-1.5` density, color-graded confidence (≥85 green-bold, 40-84 amber, <40 red) via new `classifyRowConfidence`, new `MappingSummaryStrip` replacing `WipBanner` + `CountersRow`, sticky `FilterRow`, column-template invariant test (Q10.2) | ✅ shipped 2026-04-26 |
| 4-polish-2 | Group collapse/expand state — per-project `localStorage` keying (Q3.3), auto-expand on filter match (Q3.1), no global expand-all/collapse-all v1 (Q3.2) | ⏳ pending |
| 4-polish-3 | Inline action affordances — approve/reject buttons in row actions cell, reject popover (Q5.1), deep-link to drawer for un-acknowledge (Q5.2), source swap toast (Q4.1), full Heritage MCP smoke (Q10.1, Q12.2) | ⏳ pending |
...
**Phase 4c closure summary:** With 4c-2 shipped on 2026-04-26, Phase 4c is complete. Both halves of W5 (bulk operations) are live on the redesigned Mapping page: per-target-table bulk approve and project-wide high-confidence approve from 4c-1, plus per-target-table bulk reject from 4c-2. The kebab menu in `TargetTableGroup` carries both items (Approve in gray, Reject in red, separator between). The `BulkConfirmDialog` primitive is fully exercised in both modes; the lifted-dialog state machine in `MappingContent` handles all three bulk paths (approve table / approve high-confidence / reject table) through a single discriminated union. Bulk reject takes the forward-progress partial-success path: per-TFM `resetFieldTransform` loop runs first, failed TFMs go into `failedTfmIds` and are excluded from the rejectable set, the bulk `DELETE.in()` runs against the survivors, and the toast distinguishes full from partial success. A single `mapping_bulk_rejected` activity-log entry covers the whole click. See `docs/features/mapping-redesign.md` → "Phase 4c-1 — bulk approve" and "Phase 4c-2 — bulk reject" for the full disposition.
