...
const TARGET_REASONS = [
  { value: 'auto_generated', label: 'Auto-generated by target' },
  { value: 'default_value', label: 'Will use default value' },
  { value: 'execution_package', label: 'Handled in Execution Package' },
  { value: 'will_be_null', label: 'Will be NULL' },
]

const SOURCE_REASONS = [
  { value: 'not_needed', label: 'Not needed in target' },
  { value: 'replaced_by_target', label: 'Replaced by target logic' },
  { value: 'out_of_scope', label: 'Out of scope' },
]

function AcknowledgeDropdown({
  fieldId,
  projectId,
  side,
  onDone,
  ...
  const reasons = side === 'target' ? TARGET_REASONS : SOURCE_REASONS
  ...
  const handleSelect = async (reason: string) => {
    setLoading(true)
    try {
      await acknowledgeField(projectId, fieldId, side, reason)
      onDone?.()
    } finally {
      setLoading(false)
      setOpen(false)
  ...
  return (
    <>
      <button
        ...
        onClick={handleOpen}
        disabled={loading}
        className="text-xs text-gray-500 hover:text-gray-700 font-medium px-2 py-1 rounded hover:bg-gray-100 border border-gray-200 transition-colors whitespace-nowrap"
      >
        {loading ? '…' : 'Acknowledge ▾'}
      </button>
      {open && typeof document !== 'undefined' && createPortal(
        <div
          ref={menuRef}
          className="fixed bg-white border border-gray-200 rounded-lg shadow-lg z-[200] py-1 w-56"
...
function MappingProgress({
  ...
  const totalTargetFields = Object.entries(allFieldsByTable)
    ...
    .reduce((sum, [, fields]) => sum + fields.length, 0)
  const totalFields = totalSourceFields + totalTargetFields

  const mappedSourceIds = new Set(
    allFMs.filter((fm) => fm.status !== 'rejected').map((fm) => fm.source_field_id)
  )
  const mappedTargetIds = new Set(
    allFMs.filter((fm) => !fm.is_contributing && fm.status !== 'rejected').map((fm) => fm.target_field_id)
  )
  const acknowledgedIds = new Set(acknowledgments.map((a) => a.field_id))

  let addressedSource = 0
  for (const [tid, fields] of Object.entries(allFieldsByTable)) {
    if (!sourceTableIds.has(tid)) continue
    for (const f of fields) {
      if (mappedSourceIds.has(f.id) || acknowledgedIds.has(f.id)) addressedSource++
    }
  }
  let addressedTarget = 0
  for (const [tid, fields] of Object.entries(allFieldsByTable)) {
    if (!targetTableIds.has(tid)) continue
    for (const f of fields) {
      if (mappedTargetIds.has(f.id) || acknowledgedIds.has(f.id)) addressedTarget++
    }
  }

  const totalAddressed = addressedSource + addressedTarget
  const progressPct = totalFields > 0 ? Math.round((totalAddressed / totalFields) * 100) : 0
  const isComplete = totalAddressed >= totalFields
  const remaining = totalFields - totalAddressed
    ...
    <div className="bg-white border border-gray-200 rounded-xl px-5 py-4 space-y-3">
      <div className="flex items-center justify-between">
        ...
        <div className="text-sm text-gray-600 whitespace-nowrap">
          {isComplete ? (
            <>
              <span className="font-semibold text-gray-900">{totalAddressed}/{totalFields}</span>
              <span className="text-gray-500"> fields addressed</span>
      ...
      <div className="flex items-center gap-6 text-sm">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className={addressedSource >= totalSourceFields ? 'text-green-700' : 'text-gray-600'}>
            {addressedSource}/{totalSourceFields} source fields
          </span>
        </div>
      </div>
    </div>
            ...
            <div className="border-t border-dashed border-gray-200 mt-2 pt-2">
              ...
              {unmappedTgtFields.map((field) => {
                ...
                return (
                  <div key={field.id}>
                    {ack ? (
                      <div className="flex items-center px-5 py-2 bg-green-50/50 transition-colors">
                        ...
                        <div className="w-[36%] flex items-center gap-2 min-w-0">
                          ...
                          <span className="text-sm text-gray-600 truncate">{field.name}</span>
                          <span className="text-[10px] font-medium text-green-600 bg-green-50 px-1.5 py-0.5 rounded flex-shrink-0">
                            {TARGET_REASONS.find((r) => r.value === ack.reason)?.label ?? ack.reason}
                          </span>
                        </div>
                        <div className="w-44 flex items-center justify-end flex-shrink-0">
                          <button
                            onClick={async () => { await removeAcknowledgment(projectId, field.id); onAcknowledgmentChanged(); }}
                            className="text-xs text-gray-400 hover:text-red-500 font-medium px-2 py-1 rounded hover:bg-gray-100 transition-colors"
                          >
                            Undo
                          </button>
                        </div>
                      ...
                      <div className="flex items-center px-5 py-2 hover:bg-gray-50 transition-colors text-gray-400">
                        <div className="w-[36%] flex items-center gap-2">
                        ...
                        <div className="w-44 flex items-center justify-end gap-1 flex-shrink-0">
                          <button
                          ...
                          <AcknowledgeDropdown
                            ...
                            projectId={projectId}
                            side="target"
                            onDone={onAcknowledgmentChanged}
                          />
                        </div>
                      </div>
                    )}
                    ...
                    {ack ? (
                      <div className="flex items-center px-5 py-2 bg-green-50/50 transition-colors">
                        <div className="w-[36%] flex items-center gap-2 min-w-0">
                          ...
                          <span className="text-sm text-gray-600 truncate">{field.name}</span>
                          <span className="text-[10px] font-medium text-green-600 bg-green-50 px-1.5 py-0.5 rounded flex-shrink-0">
                            {SOURCE_REASONS.find((r) => r.value === ack.reason)?.label ?? ack.reason}
                          </span>
                        </div>
                        <div className="w-[28%] text-center">
                          <span className="text-xs text-gray-300">—</span>
                      ...
                      <div className="flex items-center px-5 py-2 transition-colors text-gray-400">
                        <div className="w-[36%] flex items-center gap-2 min-w-0">
...
export default function MappingContent({ projectId, initialData }: Props) {
  ...
  // Unmapped tab: unmapped AND unacknowledged source + target fields
        ...
        <div className="flex-1 space-y-3 min-w-0">
          {filteredMappings.length === 0 ? (
            <div className="text-center py-12 text-sm text-gray-400 border border-dashed border-gray-200 rounded-xl">
              {activeFilter === 'needs_review' ? 'All mappings reviewed and all fields addressed.' : activeFilter === 'approved' ? 'No approved mappings yet.' : activeFilter === 'unmapped' ? 'All fields are mapped or acknowledged.' : activeFilter === 'many_to_one' ? 'No many-to-one mappings found.' : activeFilter === 'one_to_many' ? 'No one-to-many mappings found.' : 'No mappings found.'}
            </div>
          ) : (
            filteredMappings.map((tm) => (
              <TableMappingCard
                ...
                showInlineUnmapped={activeFilter === 'all' || activeFilter === 'unmapped' || activeFilter === 'needs_review'}
                hideMappedRows={activeFilter === 'unmapped'}
                filterVisibleStatus={activeFilter === 'needs_review' ? 'needs_review' : undefined}
                acknowledgments={acknowledgments}
                projectId={projectId}
