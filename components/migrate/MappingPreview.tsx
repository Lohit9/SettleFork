    ...
    <div ref={containerRef} className="bg-mine-slate-900 rounded-2xl overflow-hidden shadow-2xl">
      ...
      <div className="p-5">
        ...
        {rows.map((obj, i) => (
          <div
            key={i}
            className="grid grid-cols-[1fr_24px_1fr] gap-x-3 py-2.5 border-b border-mine-slate-800 transition-all duration-300"
            style={{
          ...
          >
            ...
            <span className="text-mine-slate-600 text-sm text-center">→</span>
            <span className="text-teal-300 text-sm font-mono truncate">{obj.target_object}</span>
          </div>
        ))}

        {/* Summary */}
        <div className="mt-3 pt-3 border-t border-mine-slate-700 flex justify-between items-center">
          <span className="text-mine-slate-500 text-xs">
        ...
        <div className="mt-4 text-center">
          <span className="text-mine-blue-400 hover:text-mine-blue-200 text-xs font-medium transition-colors cursor-pointer">
            See full mapping →
          </span>
        </div>
      </div>
